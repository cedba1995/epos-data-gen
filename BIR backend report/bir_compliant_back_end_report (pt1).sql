USE backupdb; -- Default schema 

-- Drop temp table if it already exists within selected schema
...
-- Populate temp table with data from epos transaction tables
INSERT INTO sales_summary 
-- The following query gets the min (beginning) and max (ending) order number per each day.
-- Computes the gross sales amount, discount, returns etc.
SELECT CONVERT(h.`ERDAT`, DATE) AS `trans_date`, CONVERT(RIGHT(MIN(h.`ORDNUM`), 10), SIGNED) AS `beginning_or_no`
	,CONVERT(RIGHT(MAX(h.`ORDNUM`), 10), SIGNED) AS `ending_or_no` 
	,SUM(d.`QTYORDERED`*d.`KBETR`) AS `gross_sales_amt`, 0
	,SUM(d.`QTYORDERED`*d.`KBETR`*d.`DISCPER`/100) AS `discount`, h.TERMINALNO, h.USERID
FROM zsd_eposordh h
INNER JOIN zsd_eposordd d ON h.`MANDT` = d.`MANDT`
			AND h.`ORDNUM` = d.`ORDNUM`
WHERE TERMINALNO = 3 -- Replace this value by the terminal number used by the last z-reading submitted by the operations dept. to BIR
GROUP BY h.`ERDAT` ;
...
WHERE s2.trans_date BETWEEN '2014-03-01' AND '2014-03-01'; -- Adjust the date if needed
...
-- Update placeholders with values from tblrefunds
